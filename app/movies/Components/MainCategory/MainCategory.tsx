...
const MainCategory: FC<Props> = ({ content, card_width, genres, title }) => {
  ...
  return (
    <div className="relative">
      <Display activeContent={activeContent} genres={genres} title={title} />
      <div className={`relative h-52 overflow-hidden`} ref={category_container}>
        <div
          className={`absolute grid grid-flow-col gap-x-2 z-0 px-8 left-0 overflow-hidden h-full`}
          style={{ translate: 0 }}
          ref={cards_container}
          id={`card_container`}
        >
          {content.results.map((obj) => {
            return (
              <Card
                ...
                content={obj}
                set_activeContent={set_activeContent}
